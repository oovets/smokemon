...
echo "==> shell helpers (/etc/profile.d/smokemon.sh)"
cat > /etc/profile.d/smokemon.sh <<EOF
# smokemon shell helpers (generated by install.sh). New login shells get smoke*.
smoke()     { PYTHONPATH=$DIR $PYTHON -m smokemon.cli "\$@"; }
smokelive() { smoke live "\$@"; }
smokekiosk(){ smoke kiosk "\$@"; }
smokepng()  { smoke png "\$@"; }
EOF
chmod 644 /etc/profile.d/smokemon.sh
