...
for f in QUICKSTART.md INSTALL.md CHANGELOG.md CONTRIBUTING.md SECURITY.md; do
    [ -f "$f" ] && cp "$f" "docs/$f"
done
...
for f in docs/*.md; do
    sed -i \
        ...
        -e 's#](\(\.\./\)*docs/detector-spec\.md)#](detector-spec.md)#g' \
        -e 's#](\(\.\./\)*docs/adr/#](adr/#g' \
...
SHELL = {"curl", "sudo", "git", "python", "python3", "pip", "brew", "cp", "sed", "awk", "for",
         "while", "ssh", "systemctl", "ss", "getcap", "mkdir", "tee", "alias",
         "export", "smoke", "smokeincidents", "journalctl", "scp", "bash",
