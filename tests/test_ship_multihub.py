...
def node_with_rows(tmp_db):
    ...
    for i in range(3):
        schema.insert(conn, "heartbeats", [{"ts": time.time(), "cpu_pct": float(i)}])
    conn.commit()
    yield conn
    conn.close()
...
def _maxid(conn):
    return conn.execute("SELECT MAX(id) FROM heartbeats").fetchone()[0]
...
def test_fanout_delivers_full_stream_to_each_hub(node_with_rows, monkeypatch):
    ...
    for url, _ in HUBS:
        ...
        assert ship._last(node_with_rows, dest, "heartbeats") == mx  # each hub's cursor at the full max
...
def test_one_hub_down_is_isolated_then_recovers(node_with_rows, monkeypatch):
    ...
    assert ship._last(node_with_rows, a, "heartbeats") == mx   # A advanced
    assert ship._last(node_with_rows, b, "heartbeats") == 0    # B untouched (isolated)

    down.clear()  # B comes back
    ship.drain(node_with_rows, HUBS)
    assert ship._last(node_with_rows, b, "heartbeats") == mx    # B caught up on its backlog
...
def test_lagging_hub_triggers_own_gather(node_with_rows, monkeypatch):
    ...
    ship._set_last(node_with_rows, a, "heartbeats", 2)  # A ahead, B fresh -> distinct frontiers
