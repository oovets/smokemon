...
def test_ordered_tables_can_exclude_a_priority_table(monkeypatch):
    monkeypatch.setattr(config, "SHIP_EXCLUDE", frozenset({"incidents"}))
    ordered = ship._ordered_tables()
    assert "incidents" not in ordered
    assert ordered[0] == "incident_samples"  # the next priority table still leads
