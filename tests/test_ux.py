...
non-utf8 terminals, and coloured output. All pure-stdlib, no rendering."""
...
def test_ascii_glyph_fallback():
    ...
    try:
        ...
        assert report._dot("dead", color=False) == "x"
...
def test_color_wraps_but_preserves_text():
    assert report._sgr("healthy", "32", True) == "\x1b[32mhealthy\x1b[0m"
    assert report._sgr("healthy", "32", False) == "healthy"
