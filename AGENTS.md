...
- never add node-side behavior that materially increases smokemon's edge footprint. new collectors must stay stdlib-only, bounded by explicit intervals/timeouts/limits, and avoid log streaming, docker log scans, broad discovery, large scrape bodies, or persistent subprocess tails.
