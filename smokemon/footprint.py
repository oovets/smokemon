...
def _ship_payload(conn: sqlite3.Connection, since: float, until: float,
                  node: str | None) -> tuple[dict, dict[str, int]]:
...
def analyze(conn: sqlite3.Connection, db: str, since: float, until: float,
            node: str | None = None) -> Footprint:
    tables: list[TableFootprint] = []
    mins: list[float] = []
    maxs: list[float] = []
    for table in schema.STD_TABLES:
    ...
    collector_rows = sum(t.rows for t in tables)
    payload, ship_counts = _ship_payload(conn, since, until, node)
