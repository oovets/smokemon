...
Stdlib only; reads /proc + /sys on Linux, a couple of `sysctl` reads on macOS. Best-effort: any
unreadable fact is simply omitted."""
...
def _sh(args: list[str], timeout: float = 3.0) -> str | None:
    try:
        out = subprocess.run(args, capture_output=True, text=True, timeout=timeout).stdout.strip()
