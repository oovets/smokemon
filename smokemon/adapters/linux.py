...
def read_net_counters() -> list[tuple[str, int, int, int, int]]:
    ...
    for line in lines[2:]:  # two header rows
        ...
        if len(fields) < 16:
            continue
        try:
            rows.append((iface, int(fields[0]), int(fields[8]),
                         int(fields[1]), int(fields[9])))  # iface, ib, ob, ip, op
        except (ValueError, IndexError):
            continue
    return rows


def read_net_errors() -> list[tuple[str, int]]:
    """[(iface, errs_plus_drops)] as cumulative counters, both directions summed.

    /proc/net/dev columns are rx: bytes packets errs drop ... (0-3), tx: bytes packets errs
    drop ... (8-11). Errors and drops are summed because for the question this feeds -- "is
    this link misbehaving" -- a dropped frame and a malformed one mean the same thing."""
    ...
    for line in lines[2:]:
        ...
        if len(fields) < 16:
            continue
        try:
            rows.append((iface, int(fields[2]) + int(fields[3])
