"""Push/webhook alerting (roadmap S4): fire an alert from detected incidents to ntfy,
...
def summarize_incidents(incidents: list[dict], node: str | None = None,
    ...
    qualify - the caller then sends nothing."""
    min_sev = config.NOTIFY_MIN_SEVERITY if min_severity is None else min_severity
    q = [i for i in incidents if i["severity"] >= min_sev]
    if not q:
        return None, None
    name = node or config.NODE
    worst = max(q, key=lambda i: i["severity"])
    title = f"smokemon {name}: {worst['klass']} (+{len(q) - 1} more)" if len(q) > 1 \
        else f"smokemon {name}: {worst['klass']}"
    lines = []
    for i in sorted(q, key=lambda i: (i["severity"], i["duration_s"]), reverse=True)[:20]:
        hhmm = datetime.fromtimestamp(i["start"]).strftime("%H:%M")
        lines.append(f"[{hhmm}] {i['klass']} {i['scope']}: {i['detail']}")
    return title, "\n".join(lines)


def alert_from_db(conn, since, until, node=None, min_severity=None, url=None) -> int:
    """Detect incidents in [since, until] and fire one summary alert if any clear the
